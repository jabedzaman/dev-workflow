...
local function worker(user_args)
    ...
    local warning_msg_title = args.warning_msg_title or 'Jabed, we have a problem'
    ...
    battery_widget = wibox.widget {
        icon_widget,
        battery_percentage_widget,  -- Add the new battery percentage widget
        layout = wibox.layout.fixed.horizontal,
    }
    -- Popup with battery info
    -- One way of creating a pop-up notification - naughty.notify
    local notification
    local function show_battery_status(batteryType, charge)
    ...
    function(widget, stdout)
        ...
        if show_current_level then
            ...
            battery_percentage_widget.text = string.format('%d%%', charge) -- Update the battery percentage text
        end
    ...
    if display_notification then
        battery_widget:connect_signal("mouse::enter", function() show_battery_status(batteryType, charge) end)
        battery_widget:connect_signal("mouse::leave", function() naughty.destroy(notification) end)
    elseif display_notification_onClick then
        battery_widget:connect_signal("button::press", function(_,_,_,button)
            if (button == 3) then show_battery_status(batteryType, charge) end
        end)
        battery_widget:connect_signal("mouse::leave", function() naughty.destroy(notification) end)
    end
